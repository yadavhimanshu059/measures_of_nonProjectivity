    ...
    def hdd(self,edge):
        ...
        if not self.is_projective(edge):
            if edge[0]>edge[1]:
                for nodex in nx.descendants(self.tree, self.root):
                    if edge[1]<nodex<edge[0]:                          
                        if not (edge[0] in nx.ancestors(self.tree, nodex)):                                                                 # if a node 'x' occurring the edge span is not dominated by the head of the edge
                            if not self.tree.nodes[nodex]['deprel']=='punct':
                                int_node=nodex
                                if not int_node==0:
                                    for edgex in self.tree.edges:
                                ...
                                if not int_node==0:
                                    for edgex in self.tree.edges:
                                        if edgex[1]==int_node:
                                            int_head=edgex[0]
                                    if nx.has_path(self.tree, int_head, edge[0]):
                                        pathx=next(nx.all_simple_paths(self.tree, int_head, edge[0], cutoff=None))              
                                        HDD=len(pathx)-1
                                    else:
                                        HDD=1
                                else:
